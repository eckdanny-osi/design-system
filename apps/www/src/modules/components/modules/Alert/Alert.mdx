...
See [WAI-ARIA Authoring Practices](https://www.w3.org/TR/wai-aria-practices/#alert) for additional context.
...
As a convenience to the developer, simple _dismissable_ behavior is possible with the `AlertUncontrolled` component (which merely wraps `Alert` and manages the `isOpen` state in a stateful component):
